...
class Memory:
    ...
    def __post_init__(self):
        self.autobiographical = [m.copy() for m in (get_character_data("autobiographical") or [])]
        self.scheduled_memories = [m.copy() for m in (get_character_data("scheduled_memories") or [])]
        self.relationship_milestones = [m.copy() for m in (get_character_data("milestones") or [])]
    ...
    def _classify_memory(self, text):
        tags = []
        if any(kw in text for kw in ["我叫", "我是", "我哥", "我姐", "我爸", "我妈", "亲哥", "亲姐"]): tags.append("人物关系")
        if any(kw in text for kw in ["喜欢", "讨厌", "爱吃", "最怕", "不喜欢"]): tags.append("用户喜好")
        if any(kw in text for kw in ["上次", "那天", "记得", "第一次", "之前"]): tags.append("重要事件")
        if any(kw in text for kw in ["今天", "食堂", "天气", "刚才", "这会儿"]): tags.append("临时话题")
        if any(kw in text for kw in ["不算", "不是的", "我改", "撤回", "收回"]): tags.append("被覆盖")
        if not tags: tags.append("一般")
        return tags

    def get_recent_episodes(self, n=3): return [e["summary"] for e in self.episodic[-n:]]
    ...
    def add_episode_to_vector(self, episode_id: str, text: str, metadata: dict = None):
        try:
            self.collection.add(documents=[text], ids=[episode_id], metadatas=[metadata or {}])
