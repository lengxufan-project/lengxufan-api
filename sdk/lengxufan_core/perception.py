...
class Perception:
    ...
    def advance_time(self):
        ...
        event_templates = get_character_data("event_templates") or {}
        causal_chain = get_character_data("causal_chains") or {}
        ...
        for status_key, status_val in (template.get("status_set") or {}).items():
            if status_val == "increment":
                self.status[status_key] = self.status.get(status_key, 0) + 1
            else:
        ...
        if elapsed > 30:
            ...
            if self.status.get("dream_streak", 0) > 0 and random.random() < 0.2:
                self.status["dream_streak"] -= 1
            if self.status.get("miss_wang") and random.random() < 0.3:
        ...
        for link in chain:
            if link["condition"]:
                ...
                if op == "lt" and self.emotion >= threshold: continue
                if op == "ge" and self.emotion < threshold: continue
